...
Parameters::Parameters():
        ...
        // functionreport
        PARAM_POLICY(PARAM_POLICY_ID, "--policy", "Annotation policy", "Annotation policy: 0: best e-value, 1: voting, 2: BLAST2GO-style GO hierarchy (no evidence code), 3: k-NN voting", typeid(int), (void *) &policy, "^[0-3]{1}$"),
        ...
        PARAM_KNN_K(PARAM_KNN_K_ID, "--knn-k", "k-NN k", "Policy 3: number of nearest neighbors (by best e-value) to vote among", typeid(int), (void *) &knnK, "^[1-9][0-9]*$"),
        // goenrich
...
//    combinepvalperset.push_back(&PARAM_SHORT_OUTPUT);
    ...
    functionreport.push_back(&PARAM_KNN_K);
    ...
    createfuncdb.push_back(&PARAM_FUNC_MAPPING_MODE);
...
std::vector<int> Parameters::getOutputFormat(int formatMode, const std::string &outformat, bool &needSequences, bool &needBacktrace, bool &needFullHeaders,
    ...
    for (size_t i = 0; i < outformatSplit.size(); ++i) {
        ...
        else if (outformatSplit[i].compare("pposgap") == 0){ needSequences = true; needBacktrace = true; code = Parameters::OUTFMT_PPOSGAP;}
        else if (outformatSplit[i].compare("go") == 0){ needFuncMapping = true; code = Parameters::OUTFMT_GO;}
